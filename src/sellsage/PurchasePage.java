...
public class PurchasePage extends javax.swing.JPanel {
    ...
    try {
        ResultSet resultSet = new ProductDAO().getProdStock();
        int quantity = new ProductDAO().getdaQuantity(codeText.getText());
        ...
        if (resultSet.next()) {
            ...
            int soldQuantity = Integer.parseInt(quantityText.getText());

            if (availableQuantity >= soldQuantity) {
                productDTO.setDate("02/01/2020");
                productDTO.setQuantity(soldQuantity);
                Double costPrice = Double.parseDouble(costText.getText());
                Double totalCost = costPrice * soldQuantity;
                productDTO.setTotalCost(totalCost);

                new ProductDAO().addPurchaseDAO(productDTO);
                new ProductDAO().editPurchaseStock(codeText.getText(), soldQuantity);
                new ProductDAO().editPurchaseStock(codeText.getText(), soldQuantity);
                loadDataSet();
            } else {
                JOptionPane.showMessageDialog(null, "Less items in stock than selected value.");
            }
        } else {
            JOptionPane.showMessageDialog(null, "Product not found in stock.");
        }
    } catch (SQLException e) {
        // Handle the SQL exception (e.g., log it or show an error message)
        e.printStackTrace();
    }
}
